...
{
	UINT8 name[32];				// String - This is the name of the database on the PalmPilot device.
	...
	UINT32 creationDate;		// Expressed as the number of seconds since January 1, 1904.
								//  The database will not install if this value is zero. (PalmOS 1.0.6)
	UINT32 modificationDate;	// Expressed as the number of seconds since January 1, 1904.
								//  The database will not install if this value is zero. (PalmOS 1.0.6)
	UINT32 lastBackupDate;		// Expressed as the number of seconds since January 1, 1904.
								//  This can be left at zero and the database will install.
	UINT32 modificationNumber;	// Set to zero.
	UINT32 appInfoArea;			// The byte number in the PDB file (counting from zero) at which the AppInfoArea is located.
	...
	UINT32 sortInfoArea;		// The byte number in the PDB file (counting from zero) at which the SortInfoArea is
	...
	UINT8 databaseType[4];		// String - Set this to the desired value. Generally it should match the Database Type
								//  used by the corresponding application This is 4 characters long and does not have a
								//  terminating null.
	UINT8 creatorID[4];			// String - Set this to the desired value. Generally it should match the Creator ID used
								...
								//  null.
	UINT32 uniqueIDSeed;		// This is used to generate the Unique ID number of subsequent records. Generally,
								//  this should be set to zero.
	UINT32 nextRecordListID;	// Set this to zero. (This does not appear to be used, but that has not been verified by
								//  a third party.)
...
struct pdb_recheader{
	UINT32 recordDataOffset;	// The byte number in the PDB file (counting from zero) at which the record is located.
	...
	UINT8	uniqueID[3];		// Set this to zero and do not try to second-guess what PalmOS will do with this value.
}
#ifdef __GNUC__
__attribute__((packed))
#endif
